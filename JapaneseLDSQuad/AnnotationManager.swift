    ...
    func generateTitlePrimary(scripture: Scripture) -> String {
        if scripture.parent_book.link.hasPrefix("gs") || scripture.parent_book.link.hasPrefix("jst") {
            if let title = getScripture(verseId: "\(scripture.id.prefix(4))title") {
                return "\(title.scripture_primary.replacingOccurrences(of: Constants.RegexPatterns.Tags, with: "", options: .regularExpression).replacingOccurrences(of: "：.*", with: "", options: .regularExpression)) : \(scripture.verse)"
            }
        }
    ...
    func generateTitleSecondary(scripture: Scripture) -> String {
        if scripture.parent_book.link.hasPrefix("gs") || scripture.parent_book.link.hasPrefix("jst") {
            if let title = getScripture(verseId: "\(scripture.id.prefix(4))title") {
                ...
                return "\(titleSecondary.replacingOccurrences(of: Constants.RegexPatterns.Tags, with: "", options: .regularExpression).replacingOccurrences(of: ":.*", with: "", options: .regularExpression)) : \(scripture.verse)"
            }
        }
